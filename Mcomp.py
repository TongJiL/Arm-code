...
class elements(ExplicitComponent):
    ...
    def compute(self,inputs,outputs):
        ...
        outputs['G_q']=np.array([[(p_4)*(np.cos(q_1))],[(p_5)*(np.cos(q_2))]])
        outputs['M_q']=np.array[p_1,(p_3)*cos(q_1-q_2)
                                (p_3)*cos(q_1-q_2),p_2]
        outputs['C_q']=np.array[0,(p_3)*sin(q_1-q_2)*w_2
                                -(p_3)*sin(q_1-q_2)*w_1,0]
        outputs['K_q']=np.array[-k*(q_2-q_1-E_k)
                                (q_2-q_1-E_k)*k]
        outputs['F']=np.array[[tor],[0]]
